  ...
  template: `
    ...
    @if (showBackToTop()) {
      <button (click)="scrollToTop()"
              ...
              class="fixed bottom-[90px] md:bottom-28 left-4 md:left-6 z-40 w-10 h-10 bg-white border border-gray-200 text-gray-500 hover:text-blue-600 hover:border-blue-300 rounded-full shadow-md hover:shadow-lg flex items-center justify-center transition-all duration-300 hover:-translate-y-1 animate-slide-up">
        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
    ...
    @if (showInstallBanner()) {
      <div class="md:hidden fixed bottom-[100px] left-3 right-3 z-50 animate-slide-up">
    ...
    <div class="md:hidden fixed bottom-0 inset-x-0 z-40"
      ...
      <div class="grid grid-cols-4 h-[62px]">

        <!-- 1 · Call -->
        <a [href]="'tel:+' + clinic.config.phoneE164"
           class="flex flex-col items-center justify-center gap-[5px] text-gray-400 active:text-[var(--accent)] transition-colors">
          <svg class="w-[22px] h-[22px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
          </svg>
          <span class="text-[9.5px] font-semibold leading-none">Call</span>
        </a>
        ...
        <a routerLink="/appointment"
          ...
          <svg class="w-[20px] h-[20px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
            <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
          </svg>
          <span class="text-[9.5px] font-bold leading-none">Book</span>
        </a>
        ...
        <button (click)="openAiChat()"
                class="flex flex-col items-center justify-center gap-[5px] text-gray-400 active:text-[var(--accent)] transition-colors">
          <div class="relative">
            <svg class="w-[22px] h-[22px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75">
        ...
        <!-- 4 · WhatsApp -->
        <a [href]="clinic.bookingWhatsappUrl" target="_blank" rel="noopener noreferrer"
           class="flex flex-col items-center justify-center gap-[5px] text-gray-400 active:text-green-600 transition-colors">
          <svg class="w-[22px] h-[22px]" fill="currentColor" viewBox="0 0 24 24">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347M12 0C5.373 0 0 5.373 0 12c0 2.117.549 4.104 1.508 5.835L0 24l6.335-1.484A11.94 11.94 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0z"/>
          </svg>
          <span class="text-[9.5px] font-semibold leading-none">WhatsApp</span>
        </a>
...
export class ClinicLayoutComponent implements OnInit, OnDestroy {
  ...
  openAiChat() { this.voiceAgent?.startText(); }
