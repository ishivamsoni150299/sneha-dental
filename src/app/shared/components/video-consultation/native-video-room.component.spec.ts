...
describe('NativeVideoRoomComponent', () => {
  ...
  beforeEach(() => {
    room = jasmine.createSpyObj<Room>('Room', ['on', 'connect', 'disconnect', 'removeAllListeners'], {
      remoteParticipants: new Map(), localParticipant: { publishTrack: jasmine.createSpy('publishTrack').and.resolveTo() } as unknown as Room['localParticipant'],
    });
    ...
    sdk = {
      ...
      VideoPresets: { h720: { resolution: {} } }, RoomEvent: {}, ConnectionQuality: { Poor: 'poor' },
  ...
  it('does not connect if dismissed while refreshing access', async () => {
    ...
    const pending = view.componentInstance.join(); await Promise.resolve();
