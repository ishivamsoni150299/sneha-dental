import { normalizePatientPhone } from './patient-auth.service';
...
describe('PatientAuthService phone helpers', () => {
  it('never treats a locally stored phone as verification', () => {
    localStorage.setItem('patient-booking-phone', '+919876543210');
    const auth = { role: signal<AuthRole | null>(null), currentUser: signal<PlatformUser | null>(null), authReady: Promise.resolve() };
  ...
  it('normalizes supported Indian mobile formats', () => {
    expect(normalizePatientPhone('9876543210')).toBe('+919876543210');
    expect(normalizePatientPhone('+919876543210')).toBe('+919876543210');
    expect(normalizePatientPhone('98765 43210')).toBeNull();
