 ...
 * POST /api/send-email
 *
 * Internal Vercel function for all transactional emails.
 * Called server-side only — never from the browser.
 *
 * Required env vars (set in Vercel dashboard):
 *   ZOHO_SMTP_USER    → mydentalplatform@zohomail.in
 *   ZOHO_SMTP_PASS    → app-specific password from Zoho Mail settings
 *   INTERNAL_API_KEY  → a random secret so only your own API routes can call this
 *
...
// ── SMTP transporter (created once per cold-start) ────────────────────────────
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Protect this endpoint — only internal server-side callers may use it
  ...
  const { template, to, data } = req.body as SendEmailPayload;

  if (!template || !to || !data) {
    return res.status(400).json({ error: 'template, to, and data are required' });
  }
  ...
  try {
    ...
    console.error(`[send-email] Failed to send ${template} to ${to}:`, msg);
    return res.status(500).json({ error: msg });
  }
}

// ── Reusable helper for other API routes ─────────────────────────────────────

/**
 * Call this from other Vercel API routes to send emails without an HTTP round-trip.
 * Requires the same env vars as the handler above.
 */
export async function sendEmail(template: EmailTemplate, to: string, data: Record<string, string>): Promise<void> {
  if (!to?.includes('@')) return; // silently skip if no email
